...
export default function Room() {
  ...
  return (
    <div className="h-screen bg-slate-950 flex flex-col text-slate-50 overflow-hidden select-none font-sans">
      ...
      <div className="h-16 px-4 md:px-6 flex items-center justify-between border-b border-slate-900 bg-slate-950/50 backdrop-blur-xl shrink-0">
        <div className="flex items-center space-x-3">
          ...
          <div className="flex flex-col">
            <h1 className="font-bold text-[13px] tracking-tight text-slate-200 uppercase leading-none mb-1">Nexus Meeting</h1>
            <div className="flex items-center space-x-2">
               <span className="text-[10px] font-mono text-slate-500 uppercase tracking-widest leading-none">{roomId}</span>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        <div className="flex-1 p-3 md:p-6 flex flex-col items-center justify-center relative overflow-y-auto overflow-x-hidden">
          <div className={cn("w-full h-full max-w-6xl mx-auto grid gap-3 md:gap-5 content-center justify-items-center", participants.size === 0 ? "grid-cols-1 max-w-3xl" : participants.size === 1 ? "grid-cols-1 md:grid-cols-2" : participants.size === 2 ? "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3" : "grid-cols-2 lg:grid-cols-4")}>
            <div className="relative w-full rounded-2xl md:rounded-[2rem] bg-slate-900 border border-slate-800/50 overflow-hidden aspect-video shadow-2xl transition-all duration-500 hover:border-indigo-500/30">
              <video ref={localVideoRef} autoPlay muted playsInline className={cn("w-full h-full object-cover", !isScreenSharing && "scale-x-[-1]")} />
              <div className="absolute bottom-3 left-3 bg-slate-950/80 backdrop-blur-2xl px-2.5 py-1 rounded-2xl text-[9px] font-bold border border-white/10 flex items-center space-x-1.5 shadow-2xl">
            ...
            {Array.from(participants.entries()).map(([id, p]) => (
              <div key={id} className="relative w-full rounded-2xl md:rounded-[2rem] bg-slate-900 border border-slate-800/50 overflow-hidden aspect-video shadow-2xl flex items-center justify-center transition-all duration-500 hover:border-indigo-500/30">
                {p.status === 'connecting' ? <div className="flex flex-col items-center space-y-3 text-slate-600 animate-pulse"><div className="w-10 h-10 border-2 border-indigo-500/20 border-t-indigo-500 rounded-full animate-spin" /><span className="text-[8px] font-black uppercase tracking-[0.3em]">Connecting</span></div> : <video ref={v => { if (v) v.srcObject = p.stream }} autoPlay playsInline className="w-full h-full object-cover" />}
                <div className="absolute bottom-3 left-3 bg-slate-950/80 backdrop-blur-2xl px-2.5 py-1 rounded-2xl text-[9px] font-bold border border-white/10 shadow-2xl"><span className="tracking-tight text-slate-300">{p.name}</span></div>
              </div>
            ))}
          </div>
        </div>
        {(showChat || showParticipants) && (
          <div className="w-full h-full md:w-80 border-l border-slate-900 bg-slate-950/90 backdrop-blur-2xl flex flex-col absolute md:relative z-[60] inset-0 md:inset-auto animate-in slide-in-from-right duration-500 shadow-2xl">
            ...
            <div className="flex-1 overflow-hidden flex flex-col">
              {showChat ? (
                <>
                  <ScrollArea className="flex-1 p-4"><div className="space-y-5">{messages.map((m, i) => (<div key={i} className={cn("flex flex-col space-y-1", m.userId === socketRef.current?.id ? "items-end" : "items-start")}><div className="flex items-center space-x-2 px-1"><span className="text-[8px] font-black text-slate-600 uppercase">{m.userName}</span></div><div className={cn("text-xs px-3 py-2 rounded-2xl max-w-[90%] break-words shadow-sm font-medium", m.userId === socketRef.current?.id ? "bg-indigo-600 text-white rounded-tr-none" : "bg-slate-900 border border-slate-800 text-slate-300 rounded-tl-none")}>{m.message}</div></div>))}</div></ScrollArea>
                  <div className="p-4 border-t border-slate-900 bg-slate-950/50"><form className="flex space-x-2" onSubmit={e => { e.preventDefault(); if(msgInput.trim()) { socketRef.current?.emit('chat-message', { roomId, message: msgInput, userName }); setMsgInput(''); } }}><Input value={msgInput} onChange={e => setMsgInput(e.target.value)} placeholder="Type..." className="bg-slate-900 border-slate-800 text-xs rounded-xl h-10" /><Button type="submit" size="icon" className="bg-indigo-600 h-10 w-10 shrink-0 rounded-xl" disabled={!msgInput.trim()}><Send className="h-4 w-4" /></Button></form></div>
                </>
              ) : (
                <ScrollArea className="flex-1 p-4"><div className="space-y-3"><ParticipantRow name={userName + " (You)"} isMe isMuted={isMuted} isVideoOff={isVideoOff} />{Array.from(participants.values()).map((p, i) => <ParticipantRow key={i} name={p.name} />)}</div></ScrollArea>
              )}
            </div>
          </div>
        )}
      </div>
      <div className="h-20 md:h-24 px-4 md:px-8 bg-slate-950/90 border-t border-slate-900 flex items-center justify-between shrink-0 relative z-[70] safe-bottom backdrop-blur-3xl">
        <div className="hidden lg:flex w-40 flex-col"><span className="text-[8px] font-black text-slate-600 uppercase tracking-widest mb-1">Time</span><span className="text-xs font-mono text-indigo-400 font-bold">00:12:45</span></div>
        <div className="flex items-center space-x-3 md:space-x-6 mx-auto">
          <Button size="icon" variant={isMuted ? "destructive" : "secondary"} className="rounded-2xl w-10 h-10 md:w-14 md:h-14 transition-all shadow-xl" onClick={() => { setIsMuted(!isMuted); localStreamRef.current!.getAudioTracks()[0].enabled = isMuted }}>{isMuted ? <MicOff className="h-4 w-4" /> : <Mic className="h-4 w-4" />}</Button>
          <Button size="icon" variant={isVideoOff ? "destructive" : "secondary"} className="rounded-2xl w-10 h-10 md:w-14 md:h-14 transition-all shadow-xl" onClick={() => { setIsVideoOff(!isVideoOff); localStreamRef.current!.getVideoTracks()[0].enabled = isVideoOff }}>{isVideoOff ? <VideoOff className="h-4 w-4" /> : <Video className="h-4 w-4" />}</Button>
          <Button size="icon" variant={isScreenSharing ? "secondary" : "ghost"} className="rounded-2xl w-10 h-10 md:w-14 md:h-14 hidden sm:flex transition-all" onClick={toggleScreen}><ScreenShare className="h-4 w-4" /></Button>
          <Button size="icon" variant="destructive" className="rounded-2xl w-14 h-10 md:w-28 md:h-14 font-black shadow-xl" onClick={handleLeave}><PhoneOff className="h-4 w-4 md:mr-2" /><span className="hidden md:inline text-[9px] uppercase tracking-widest">End Call</span></Button>
        </div>
        <div className="flex items-center space-x-2 w-20 lg:w-40 justify-end">
          <Button variant="ghost" size="icon" className={cn("rounded-2xl w-10 h-10", showParticipants && "text-indigo-400 bg-indigo-500/10")} onClick={() => { setShowParticipants(!showParticipants); setShowChat(false) }}><Users className="h-4 w-4" /></Button>
          <Button variant="ghost" size="icon" className={cn("rounded-2xl w-10 h-10", showChat && "text-indigo-400 bg-indigo-500/10")} onClick={() => { setShowChat(!showChat); setShowParticipants(false) }}><MessageSquare className="h-4 w-4" /></Button>
        </div>
      </div>
...
function ParticipantRow({ name, isMe, isMuted, isVideoOff }: { name: string, isMe?: boolean, isMuted?: boolean, isVideoOff?: boolean }) {
  return (
    <div className="flex items-center justify-between p-3 rounded-2xl bg-slate-900/40 border border-slate-800/50 transition-all hover:bg-slate-900/60 group">
      <div className="flex items-center space-x-3">
